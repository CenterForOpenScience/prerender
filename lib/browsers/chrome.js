...
const URLResponse403 = process.env.URL_RESPONSE_403;
	...
	Network.requestWillBeSent((params) => {
		...
		if (params.redirectResponse) {
			...
			if (tab.prerender.initialRequestId === params.requestId && !tab.prerender.followRedirects && !this.options.followRedirects) {
				...
				if (URLResponse403 && requestURL.includes(URLResponse403)) {
					util.log(`Redirect to login detected due to private or deleted target, so responding with 403 at request of Google Scholar Team`);
					tab.prerender.statusCode = 403;
				} else {
